...
class albuns extends MX_Controller {
	...
	public function index()
	{
		...
		foreach($query->result() as $row)
		{
			...
			$this->table->add_row(
				...
				anchor('admin/albuns/delete/'.$row->id, glyphicon('trash'), array('class' => 'btn btn-default', 'onClick'=>'return apagar();')).
				div(null,true)
			);
		}

		$content_vars['listagem'] = $this->table->generate();
		$layout_vars['content'] = $this->load->view('albuns_index', $content_vars, TRUE);
	...
	public function add()
	{
		...
		{
			$layout_vars['content'] = $this->load->view('albuns_add', $content_vars, TRUE);
			$this->load->view('layout', $layout_vars);
		} else {
	...
	public function edit($id = null)
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$layout_vars['content'] = $this->load->view('albuns_edit', $content_vars, TRUE);
			$this->load->view('layout', $layout_vars);
